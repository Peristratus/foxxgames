...
Submit btn | SB-7.1 | Gain access to Foxx games website | **As a** user **I want** to access to the contact us section**so that** I can access to the submit btn.| **Given** that the user is on Foxx Games payment web page, **When** they click on the submit btn **then** the system will submit via emailjs the users inputs and message. |    M   |
